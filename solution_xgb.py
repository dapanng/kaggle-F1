...
xgb_params = {
    ...
    'colsample_bylevel': 0.7, 'reg_alpha': 0.3, 'reg_lambda': 0.5,
    'gamma': 0.1, 'tree_method': 'hist', 'random_state': SEED,
    'n_jobs': 2, 'verbosity': 0, 'early_stopping_rounds': 100,
}
...
for fold, (tr, val) in enumerate(skf.split(X, y)):
    ...
    print(f"Fold {fold+1}: AUC={roc_auc_score(y_val, oof[val]):.6f}, Iters={model.best_iteration}")

print(f"\nXGBoost OOF AUC: {roc_auc_score(y, oof):.6f}")
np.save('oof_xgb_v1.npy', oof)
np.save('test_xgb_v1.npy', test_pred)
print("Saved oof_xgb_v1.npy, test_xgb_v1.npy")